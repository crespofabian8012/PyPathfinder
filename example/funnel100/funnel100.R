rm(list = ls())
setwd("C:\\Users\\FC7458\\Downloads\\Pathfinder\\posteriordb")
#setwd(file.path(".", "posteriordb")) # set working dir to cloned package
library(rstan)
library(parallel)
library(foreach)
...
## Compare ##
sum_des <- function(s){
  round(c(mean(s), sd(s), quantile(s, c(0.5, 0.05, 0.95))), digits = 2)
}

compare_params_graph2 = function(pf_draws, pf_draws2, ref_draws, ref_name, n_bins = 20) {
  # pf_draws = pick_samples_IR
  # pf_draws2 = pick_samples_IR2
  # ref_draws = unconstrained_draws
  # ref_name = "nuts"
  
  orig_names = colnames(ref_draws)
  
  rownames(pf_draws) <- orig_names
  rownames(pf_draws2) <- orig_names
  
  
  for (base_param_name in orig_names[c(1, 2, 9, 10)]) {
    axis_limits = range(pf_draws[base_param_name, ], 
                        pf_draws2[base_param_name, ],
                        ref_draws[, base_param_name])
    
    n_ref = nrow(ref_draws)
    n1 = ncol(pf_draws)
    n2 = ncol(pf_draws2)
    param_dt = data.frame(value = c(ref_draws[, base_param_name],
                                    pf_draws[base_param_name, ],
                                    ref_draws[, base_param_name],
                                    pf_draws2[base_param_name, ]),
                          type = c(rep(1, n_ref), 
                                   rep(2, n1),
                                   rep(1, n_ref), 
                                   rep(2, n2)),
                          test = c(rep(1, n_ref + n1), 
                                   rep(2, n_ref + n2)))
    param_dt$type <- factor(param_dt$type, levels = c(1, 2), 
                            labels = c("nuts", "Pathfinder"))
    param_dt$test <- factor(param_dt$test, levels = c(1, 2), 
                            labels = c("old", "new"))
    base_plot = param_dt %>% ggplot(aes(x=value, fill=type)) + 
      geom_histogram(bins = n_bins, color="#e9ecef", alpha=0.6, position = 'identity') +
      scale_fill_manual(values=c("#69b3a2", "#404080")) +
      theme_bw(base_size = 18) + xlim(axis_limits) +
      #ggtitle(paste0(base_param_name)) +
      labs(fill="") + facet_wrap(~ test, nrow = 1) + theme(legend.position="bottom")
    
    sumtable <- matrix(NA, nrow = 3, ncol = 5)
    colnames(sumtable) <- c("mean", "sd", "median", "q5", "q95")
    rownames(sumtable) <- c("nuts", "pathfinder old", "pathfinder new")
    sumtable[1, ] <- sum_des(ref_draws[, base_param_name])
    sumtable[2, ] <- sum_des(pf_draws[base_param_name, ])
    sumtable[3, ] <- sum_des(pf_draws2[base_param_name, ])
    sumtable.p <- ggtexttable(sumtable, #rows = NULL, 
                              theme = ttheme("mOrange"))
    
    p_joint <- ggarrange(base_plot, sumtable.p, ncol = 1, nrow = 2, 
                         heights = c(1, 0.4))
    
    p_joint2 <- 
      annotate_figure(p_joint,
                      top = text_grob(paste0("Comparison of ", base_param_name),
                                      color = "black", face = "bold", size = 14))
    print(p_joint2)
  }
}

objfun_trust <- function(fn, gr) {
  #TODO
  #compute the Hessian approximation B and return it
  list(value = fn, gradient = gr, hessian = B)
}
# Wasserstein distance checking #
library(transport)
compare_W_1_d <- function(pf_draws, pf_draws2, ref_draws){
  n1 = ncol(pf_draws)
  n2 = ncol(pf_draws2)
  nr = nrow(ref_draws)
  a1 <- wpp(t(pf_draws), mass = rep(1/n1, n1))
  a2 <- wpp(t(pf_draws2), mass = rep(1/n2, n2))
  b <- wpp(ref_draws, mass = rep(1 / nr, nr))
  compar_result <- c(wasserstein(a1, b, p = 1), wasserstein(a2, b, p = 1))
...
opt_path <- function(init, fn, gr, 
                     ...
                     eval_lp_draws = TRUE, ext_path = FALSE, 
                     hm_explore = FALSE) {
  ...
  #' @param ext_path    logical; if TRUE, extend the optimization trajectory towards the tail of the log density
  #' @param hm_explore  logical; if TRUE, generate Hamiltonian search path from initials and reinitialize the Pathfinder randomly along the search path
  #' @return 
  ...
  if(hm_explore){
    ...
    q <- init        # location
    p <- rnorm(D) * 2   # momentum
    n_leapfrog <- 800; # leapfrog
    ...
    init = q_tr[[sample.int(n_leapfrog, 1)]]
  }
  ...
  LBFGS_fail <- TRUE
  N_try <- 0 
  while(LBFGS_fail & N_try < 30){   # if L-BFGS cannot run with the initial, find another inital
    LBFGS_fail <- FALSE
    if(N_try == 0){
      N_try = N_try + 1
      y[1, 1:D] <- init
      ...
      print("\n reinitialize \n")
      N_try = N_try + 1
      if(hm_explore & N_try < 30){
        y[1, 1:D] = q_tr[[sample.int(n_leapfrog, 1)]]
      }else{
    ...
    if(LBFGS_fail){ # fail the section of code that does the checking
      ...
      next}
    if(tt$convcode == 9999){
      #LBFGS_fail <- TRUE     # still use the optimization path even when convcode = 9999
      splited_output = unlist(lapply(my_data, f <- function(x){
        strsplit(as.character(x),split = " ")}))
      
      line_search_ind = which(splited_output == "SEARCH") + 1 # check line search times the 
  ...
  if(ext_path){
    # Take optimization towards the maximum
    my_data_ext <- capture.output(
      tt_ext <- optimx(par = y[1, 1:D],
                       fn = fn,  
                       gr = gr,
                       method = "CG",#"CG", #"L-BFGS-B",
                       control = list(fnscale = -1, # negate for maximization
                                      maxit = N1, 
                                      pgtol = 0.0, 
                                      factr = factr_tol,
                                      trace = 6, REPORT = 1, lmm = lmm, 
    ...
    L_ext = length(my_data_ext); L_ext
    splited_output_ext = unlist(lapply(my_data_ext, f <- function(x){
      strsplit(as.character(x),split = " ")}))
    
    X_ind_ext = which(splited_output_ext == "parameters")[-1]
    if(length(X_ind_ext) > 0){
      Iter_ext = length(X_ind_ext);
      X_ind_ext = c(X_ind_ext, which(splited_output_ext == "$fevals"))
      X_ext = matrix(NA, nrow = Iter_ext, ncol = D)
      G_ext = matrix(NA, nrow = Iter_ext, ncol = D)
      for(g in 1:Iter_ext){
        #cat(g, "\t")
        x_raw <- as.numeric(splited_output_ext[seq(from = X_ind_ext[g]+1, to = X_ind_ext[g+1]-1, by = 1)])
        X_ext[Iter_ext - g + 1, ] = x_raw[!is.na(x_raw)][1:D]
        G_ext[Iter_ext - g + 1, ] = gr(X_ext[Iter_ext - g + 1, ])
      }
      # expand the optimization trajectory 
      X = rbind(X_ext, X)
      G = rbind(G_ext, G)
    }
  }
  ...
  y = cbind(X, -fn_ls)              # store the optimization trajectory and lp_
  ...
  # pre-allocation for all approximations
  sample_pkg_list <- list() # save the sample info for each mode
  DIV_fit_list <- list() # save the divergence related info for each mode
  ...
  for (iter in 1:Iter){
    ...
    sample_pkg_list[[iter]] <- sample_pkg   # save the sample_pkg to check
    DIV_label = "ELBO"; 
    DIV_fit <- est_DIV(sample_pkg, N_sam_DIV, fn, label = DIV_label) #lCDIV #lADIV  #lIKL #ELBO
    DIV_fit_list[[iter]] <- DIV_fit      # save the sample_pkg to check
  ...
  proc.time() -t 
  #145...
  #plot(1:length(DIV_ls), DIV_ls, ylim = c(-500, 2000))
  ...
  return(list(sample_pkg_save = sample_pkg_save, 
              ...
              fn_call = fn_call, 
              gr_call = gr_call,
              sample_pkg_list = sample_pkg_list, # save for sampling among all approximation
              DIV_fit_list = DIV_fit_list,  #save for sampling among all approximation
              status = "samples"))
}

opt_path_save_trajectory<- function(init, fn, gr, 
                     init_bound = 2.0,
                     N1 = 1000,
                     N_sam_DIV = 5, N_sam = 100,
                     factr_tol = 1e2, lmm = 6, seed = 1234,
  ...
  LBFGS_fail <- TRUE
  N_try <- 0
  while(LBFGS_fail){   # if L-BFGS cannot run with the initial, find another initial
    LBFGS_fail <- FALSE
    if(N_try == 0){
      N_try = N_try + 1
      y[1, 1:D] <- init
  ...
  Ykt_h <- NULL; Skt_h <- NULL # initialize matrics for storing history of updates
  t <- proc.time()
  list_approx_inv_hessian<-list() # list of matrices
  list_DIV_fit <- list()
  for (iter in 1:Iter){
    #cat(iter, "\t")
    inc_flag <- check_cond(Ykt[iter, ], Skt[iter, ])
    ...
    tryCatch(
      ...
      sample_pkg <- Form_N_apx_taylor(X[iter + 1, ], G[iter + 1, ], Ykt_h, Skt_h, E, lmm),
      #sample_pkg <- Form_N_apx(X[iter + 1, ], Ykt_h, Skt_h, E, lmm),
      error = function(e) { sample_pkg<<- NULL
                            ill_distr <<- TRUE})
    list_approx_inv_hessian[[iter]] <- sample_pkg
  ...
  if(is.null(DIV_ls) | (max(DIV_ls) == -Inf)){                     # if no normal approximation generated, return the set of parameters found by optim
    return(list(sample_pkg_save = sample_pkg_save,                 # matrics for estiamted inv Hessian 
                ...
                fn_call = fn_call,                                 # no. calls of log density
                gr_call = gr_call,                                 # no. calls of gradient
                list_approx_inv_hessian = list_approx_inv_hessian, # list approx inv hessian
                list_DIV_fit = list_DIV_fit,                       # list of DIV fit
                status = "mode"                                    # status of the one-path pathfinder
    ))
  }
...
opt_path_stan_save_trajectory_parallel <- function(seed_init, seed_list, mc.cores, model, data, 
  ...
  if (Sys.info()['sysname']=="Windows"){
    ...
    # library("parallel")
    #  cl <-
    # parallel::makeCluster(mc.cores, outfile = "")
    #  on.exit(parallel::stopCluster(cl), add = FALSE)
    #  doParallel::registerDoParallel(cl)
    
    
    #   cl <-clusterExport(cl, "opt_path")
    #   cl <- clusterExport(cl, "seed_list")
    
    
    #   cl <- clusterExport(cl, "seed_list")
    
    #  out <- parallel::parLapply(cl, list_ind, f <- function(x){
...
opt_path_trust_region <- function(init, fn, gr, 
                                     init_bound = 2.0,
                                     N1 = 1000,
                                     N_sam_DIV = 5, N_sam = 100,
                                     factr_tol = 1e2, lmm = 6, seed = 1234,
  ...
  #' Run one-path Pathfinder for initialization init and save the trajectory
  #' 
  #' @param init        initial parameter values
  #' @param fn          negative of log-density 
  #' @param gr          gradient function of negative log-density 
  ...
  while(LBFGS_fail){   # if L-BFGS cannot run with the initial, find another initial
    ...
    if(N_try == 0){
      N_try = N_try + 1
      current_index<-1
      y[current_index, 1:D] <- init
    }else{
      print("\n reinitialize \n")
      N_try = N_try + 1
      current_index<-1
      y[current_index, 1:D] <- runif(D, -init_bound, init_bound)
    }
    tryCatch(y[current_index, D + 1] <- -fn(y[current_index, 1:D]), 
             error = function(e) { LBFGS_fail <<- TRUE})
    if(LBFGS_fail | is.infinite(y[current_index, D + 1])){
      LBFGS_fail <- TRUE
      fn_call = fn_call + 1  # record the evaluation of log-density of ill initials
      next}
    g1 <- gr(y[1, 1:D]) # record the gradient of initials
    ...
    tryCatch(
      my_data <- capture.output(
        tt <- trust(objfun = fn, parinit = y[current_index, 1:D],
                     fn = fn,  # this function has to return a list of 3 functions: value = fn, gradient = gr, and hessian
                     rinit= 0.001,
                     minimize = FALSE),
        type = "output"), 
      error = function(e) { LBFGS_fail <<- TRUE})
...
fit_info_DIV <- function(param_path) {
  # list_DIV_fit fitting information #
...
Imp_Resam_WOR <- function(param_path, n_inits, seed = 123){
  ...
  ## list_DIV_fit samples and log ratios ##
...
Imp_Resam_WR_schema_N3 <- function(param_path, n_sam, fn,  seed = 123){
  ...
  gaussian_approx_all_paths <- do.call(c, unlist(lapply(param_path, function(path) path$list_DIV_fit), recursive=FALSE))
  
  number_total_gaussian_approximations <- sum(unlist(lapply(param_path, function(path) length(path$list_DIV_fit))))
  
  
  idxs_gaussian_approximations <- sample.int(number_total_gaussian_approximations, size = n_sam, replace = TRUE, 
                                                prob = rep(1.0 / number_total_gaussian_approximations, number_total_gaussian_approximations  ))
  
  n_draws_per_normal_approx<-table(idxs_gaussian_approximations)
    
  list_DIV_with_samples<- lapply(1:length(names(n_draws_per_normal_approx)), function(str_idx, gaussian_approx_all_paths, fn) {
                  idx<-as.numeric(str_idx)
                  gaussian_approx<-gaussian_approx_all_paths[idx]
                  draw_samples(n_draws_per_normal_approx[names(n_draws_per_normal_approx)==str_idx],  DIV_fit, fn) 
                  },
                  gaussian_approx_all_paths, fn)
  
  samples <- do.call("cbind", lapply(list_DIV_with_samples, function(DIV_fit) DIV_fit$repeat_draws ))
  lrms <- c(sapply(list_DIV_with_samples, function(DIV_fit) extract_log_ratio_from_DIV(DIV_fit)))
...
draw_samples<- function(n_draws, DIV_fit, fn, eval_lp_draws = TRUE) {
  
  draws_N_apx <- Sam_N_apx(sample_pkg, 
                            n_draws)

  DIV_fit$repeat_draws <- cbind(DIV_fit$repeat_draws, draws_N_apx$samples)
  DIV_fit$lp_approx_draws <- c(DIV_fit$lp_approx_draws,
                                    draws_N_apx$lp_apx_draws)
  ...
  fn_call <- 0
   if(eval_lp_draws == TRUE){
        ...
        DIV_fit$fn_draws <- c(DIV_fit$fn_draws, fn_draws_apx)
        fn_call = fn_call +  n_draws
      }
   return(DIV_fit)
...
extract_log_ratio_from_DIV <- function(DIV_fit){
  if(length(DIV_fit$fn_draws)!=
     length(DIV_fit$lp_approx_draws)){
    stop('length of log-densities for approximate draws does not match with the sample size, set option eval_lp_draws at TRUE before running importance resampling')
  }
  lrm <- - DIV_fit$fn_draws - DIV_fit$lp_approx_draws
  return(lrm)
}
...
filter_samples_N3_all <- function(param_path, n_inits, seed = 123,
                                  escape_times = 0){
  
  #' SIR with mixture Gaussian 
  
  if(escape_times > 0){
    param_path <- sapply(param_path, f <- function(x){x}) 
  }
  ...
  param_path <- param_path[filter_mode]
  D <- length(param_path[[1]]$sample_pkg_save$x_center)
  
  sample_pks_all <- 
    unlist(sapply(param_path, f1 <- function(x){x$sample_pkg_list}), recursive=F)
  ind_null_mod <- 
    sapply(sample_pks_all,f1 <- function(x){
      if(is.null(x)){TRUE}else if(x$label == "mode"){TRUE}else{FALSE}})
  samples <- unlist(sapply(param_path, f1 <- function(x){x$DIV_fit_list}), 
                    recursive=F)
  
  sample_pks_all <- sample_pks_all[!ind_null_mod]
  samples <- samples[!ind_null_mod]
  lps <- c(sapply(samples, f1 <- function(x){-x$fn_draws}))
  #lps <- c(sapply(param_path, extract_lps))
  #samples <- lapply(param_path, extract_samples)
  
  #J <- length(param_path)
  #ns <- ncol(param_path[[1]]$DIV_save$repeat_draws)
  J = length(samples)
  ns = ncol(samples[[1]]$repeat_draws)
  lp_approx_M <- matrix(0, nrow = J*ns, ncol = J) # save log-densities 
  ...
  for(indI in 1:J){  #sample
    ...
    for(indJ in 1:J){ #pathfinder
      if (indJ == indI){
        lp_approx_M[((indI-1)*ns + 1):(indI * ns), indJ] <- 
          samples[[indJ]]$lp_approx_draws
        #param_path[[indJ]]$DIV_save$lp_approx_draws
      } else {
        lps_tem <- rep(0, ns)
        if(sample_pks_all[[indJ]]$label == "full"){
          u <- forwardsolve(sample_pks_all[[indJ]]$cholHk,
                            (samples[[indI]]$repeat_draws -  
                               sample_pks_all[[indJ]]$x_center),
                            transpose = TRUE, upper.tri = TRUE)
          lps_tem = - sample_pks_all[[indJ]]$logdetcholHk -
            0.5 * colSums(u^2) - 0.5 * D * log(2*pi)
        }else{
          u1 <- (samples[[indI]]$repeat_draws - sample_pks_all[[indJ]]$x_center) * 
            sqrt(sample_pks_all[[indJ]]$theta_D)
          u2 = crossprod(sample_pks_all[[indJ]]$Qk, u1)
          norm_u <- 
            colSums(forwardsolve(sample_pks_all[[indJ]]$Rktilde, u2,
                                 transpose = TRUE, upper.tri = TRUE)^2) + 
            colSums(u1^2) - colSums(u2^2)
          lps_tem = - sample_pks_all[[indJ]]$logdetcholHk -
            0.5 * norm_u - 0.5 * D * log(2*pi)
        }
        lp_approx_M[((indI-1)*ns + 1):(indI * ns), indJ] <- lps_tem
      }
    }
  ...
  lp_approx <- apply(lp_approx_M, 1, logSumExp)
  lrms <- lps - lp_approx
  finite_lrms <- is.finite(lrms)
  
  sample_weights <- suppressWarnings(weights(psis(lrms[finite_lrms], r_eff = NA), 
                                             log = FALSE))
  
  samples_M <- matrix(sapply(samples, f <- function(x){x$repeat_draws}), 
                      nrow = D)[, finite_lrms]
  set.seed(seed)
  sample_ind <- sample(1:length(sample_weights), replace = TRUE,
                       size = n_inits, prob = sample_weights)
  print(sort(table(sample_ind), decreasing = TRUE)[1:20])
  return(samples_M[, sample_ind])
}
...
if(fit_pf_flag){
  ...
  t <- proc.time()
  opath <- opt_path_stan_save_trajectory_parallel(seed_list, seed_list, mc.cores, model, data = NULL,
  ...
  pick_samples_IR_N1 <- Imp_Resam_WR(opath, n_sam = N_sam, seed = 1)
  #pick_samples_IR_N3 <- filter_samples_resam(opath, n_inits = N_sam, seed = 1)
  #' N3 scheme for importance resampling for selected approximations from multi-path pathfinder
  pick_samples_IR_N3 <- filter_samples_resam(opath, n_inits = N_sam, seed = 1)
  #' N3 scheme for importance resampling for all approximations from multi-path pathfinder
  pick_samples_IR_N3_all <- filter_samples_N3_all(opath, n_inits = N_sam, seed = 1)
  
 
  
  
  print("Wasserstein distance N1 vs N3 all")
  compare_W_1_d(pick_samples_IR_N1, pick_samples_IR_N3_all, unconstrained_draws)
  
  print("Wasserstein distance N1 vs N3")
  
  compare_W_1_d(pick_samples_IR_N1, pick_samples_IR_N3, unconstrained_draws)
  
  print("Wasserstein distance N3 vs N3 all")
  compare_W_1_d(pick_samples_IR_N3, pick_samples_IR_N3_all, unconstrained_draws)
  
  library(ggplot2)
  library(dplyr)
  library(ggpubr)
  compare_params_graph2(pick_samples_IR_N1, pick_samples_IR_N3_all, 
                        unconstrained_draws, "nuts", n_bins= 50) 
...
pick_samples<- pick_samples_IR_N3_all
...
ifelse(!dir.exists(file.path("../pics/", "phI_adapt_default")), dir.create(file.path("../pics/", "phI_adapt_default")), FALSE)
ggsave("funnel100_points.eps", #"funnel100_points22.eps"
       plot = p_check,
       device = cairo_ps,
       path = file.path("../pics/", "phI_adapt_default"),
